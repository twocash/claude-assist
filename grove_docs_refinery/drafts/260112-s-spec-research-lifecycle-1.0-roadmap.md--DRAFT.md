## Rewrite: 260112-s-spec-research-lifecycle-1.0-roadmap.md
### Diagnosis Summary
Source document from January 2026 contains strong technical content but lacks Grove's current voice and terminology. Uses some legacy terms ("demo," generic technical language) and could better reflect Grove's exploration architecture positioning. The core architectural insights are solid and should be preserved.

### Key Changes Made
- Updated terminology (Users→Observers, demo→proof of concept)
- Strengthened opening to reflect Grove's exploration architecture positioning
- Tightened prose throughout, removing hedging and filler
- Maintained technical precision while improving accessibility
- Preserved all intentional uncertainty in appropriate sections
- Added Grove-specific framing to technical decisions

### Flags for Review
- Kept technical diagram formatting as-is (ASCII art)
- Preserved sprint structure but strengthened language
- Maintained all specific technical details and file paths
...
The "Prompt Architect" isn't a standalone entity—it's the existing intake pipeline *reading* PromptArchitectConfig to determine behavior. The "Writer Agent" isn't a separate service—it's the writing phase reading WriterAgentConfig to determine voice, structure, and citation style.
