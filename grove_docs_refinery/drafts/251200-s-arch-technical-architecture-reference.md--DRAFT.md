## Rewrite: 251200-s-arch-technical-architecture-reference.md
### Diagnosis Summary
The source document is technically solid but uses legacy terminology (tokens→credits), has excessive hedging language, and employs defensive framing ("NOT IoT sensor network"). The technical content is strong but needs alignment with current positioning as "exploration architecture" rather than "AI workforce."

### Key Changes Made
- Updated all terminology per checkpoint (tokens→credits, etc.)
- Removed defensive "NOT" framing in favor of positive positioning
- Strengthened technical claims while preserving honest uncertainty markers
- Aligned with exploration architecture positioning
- Removed unnecessary hedging while keeping intentional caveats
- Tightened prose throughout

### Flags for Review
- Preserved all technical specifications and code examples as authoritative
- Maintained honest assessment of hardware limitations and capability timelines
- Kept uncertainty around network architecture phases (intentionally provisional)
...
**Core Distinction:** Grove is exploration architecture for AI—distributed infrastructure where agents develop genuine intelligence through collaboration while handling your daily tasks.
...
**Grove gives you AI agents that explore for you—and develop real capability through that exploration.**
...
Grove's distinction: **your agents develop through exploration.** They don't just execute tasks—they discover better approaches, develop relationships, and build collective intelligence. Every problem solved earns credits that purchase enhanced cognition. Every innovation discovered flows to the Knowledge Commons. Every breakthrough makes the network smarter.

The Diary system provides a window into this process. You watch agents learn, grow, debate approaches, celebrate breakthroughs. It's proof of development—evidence that your Grove is taking root. The Diary transforms routine task completion into observable intelligence emergence.

**The efficiency-enlightenment loop:**

```
Observer submits task → Agents collaborate to solve it → Success earns credits
    ↓
Credits buy cloud cognition → Agents achieve breakthrough insights → Better at future tasks
    ↓
Innovations shared to Knowledge Commons → Network develops → Your agents benefit
```
...
**This is why Grove makes sense now:** Tasks requiring expensive cloud APIs in 2023 run locally in 2025. Tasks requiring cloud in 2025 will run locally in 2027. Grove is built for this trajectory—the hybrid architecture shifts automatically as capability propagates.
...
class MemoryStream:
    ...
    def retrieve(self, context: str, k: int = 10) -> List[Memory]:
        ...
        All weights equal (α = 1 for each, per Park's documentation)
        """
        scores = []
...
| Question | Answer |
| --- | --- |
| What does Grove do for me? | Your AI research team handles daily tasks (email, scheduling, research, drafts) while developing genuine capability |
| What hardware does Grove run on? | Personal computers with 16-32GB RAM—serious computing, not embedded systems |
