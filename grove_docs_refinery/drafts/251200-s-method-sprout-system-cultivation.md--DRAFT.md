## Rewrite: 251200-s-method-sprout-system-cultivation.md
### Diagnosis Summary
Source document from December 2024 describes the Sprout System architecture but uses outdated terminology ("chatbot" instead of "agent"), lacks Grove's current voice (too academic), and doesn't reflect evolved understanding of exploration architecture. Technical details are solid but framing needs alignment with current positioning.

### Key Changes Made
- Replaced "chatbot" with "agent" throughout
- Reframed from "collective intelligence" to "exploration architecture"
- Updated terminology: LLM → Agent, chatbot → agent interface
- Removed academic hedging while preserving substantive caveats
- Simplified language while maintaining technical precision
- Aligned with Grove's exploration thesis and current strategic positioning
- Made concrete examples more Grove-specific

### Flags for Review
- Kept "Field" terminology as it appears central to the architecture
- Preserved technical specifications verbatim (TypeScript interfaces)
- Maintained botanical metaphor as it aligns with Grove terminology
- Section 8 "Research Questions" preserved intentional uncertainty
...
|-------|-------|-------------|
| **Seed** | Ephemeral | Raw agent output, lost when conversation ends |
