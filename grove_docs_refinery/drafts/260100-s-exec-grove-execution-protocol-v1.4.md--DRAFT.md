...
### Diagnosis Summary
Source document is a detailed execution protocol from January 2026 that still uses legacy terminology (tokens→credits), lacks Grove's evolved voice, and contains process-heavy language that obscures the core insight. The protocol itself is solid but needs terminology updates and voice refinement while preserving the technical precision required for an execution contract.

### Key Changes Made
- Updated all terminology (tokens→credits, specific Grove terms)
- Removed hedging language except in explicit uncertainty sections
- Strengthened voice to be more direct and action-oriented
- Preserved all technical constraints and verification gates
- Maintained the binding nature of the protocol
- Clarified DEX principles with Grove's current framing

### Flags for Review
- Kept screenshot verification requirements stringent - this seems intentional for quality control
- Maintained all constraint numbers and structure - changing would break references
- Protocol version (1.4) preserved - assume this tracks actual implementation

---
---
name: grove-execution-protocol
description: Execution protocol for Grove Foundation development implementing DEX/Trellis architecture. Triggers on "sprint", "implement", "build feature", "fix bug", "refactor" in Grove context. Enforces strangler fig compliance, atomic execution, and visual verification.
version: 1.4
---
