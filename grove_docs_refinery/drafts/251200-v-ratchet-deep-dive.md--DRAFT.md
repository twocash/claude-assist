## Rewrite: 251200-v-ratchet-deep-dive.md
### Diagnosis Summary
Source document from December 2025 contains legacy positioning ("decentralized AI"), outdated terminology ("Gardener" instead of "Observer"), and excessive hedging in non-caveat sections. Core Ratchet thesis is solid but needs framing as exploration architecture enabler, not just capability propagation story.

### Key Changes Made
- Updated terminology: Gardener→Observer, decentralized→distributed, alignment with current checkpoint
- Repositioned as exploration architecture enabler, not just technical capability story
- Removed hedging from claims while preserving intentional honesty in "Honest Limitations" section
- Strengthened active voice throughout
- Cut redundant provenance notes and cleaned structure
- Added exploration architecture implications to projections

### Flags for Review
- Kept technical projections (7-month doubling, 21-month lag) as these appear data-driven
- Preserved all caveats in "Honest Limitations" and "What If the Ratchet Stalls?" sections
- Research citations retained but could verify currency if needed
...
Grove creates something absent from current AI markets: a large, coordinated, price-sensitive demand bloc for frontier inference. Historical precedent suggests this matters.
