## Rewrite: 260100-s-arch-bedrock-information-architecture.md
### Diagnosis Summary
Source document is recent (January 2026) and technically accurate but uses outdated terminology, especially around Grove structure. While the architectural thinking is sound, it doesn't reflect Grove's current positioning as exploration infrastructure or use updated terms from the checkpoint.

### Key Changes Made
- Updated terminology: Pipeline→Garden remains, but framed in exploration architecture context
- Replaced generic "platform ecosystem" with specific exploration infrastructure framing
- Changed "users" to "Observers" in agent-facing contexts, kept "users" in UI contexts
- Removed hedging language throughout
- Strengthened Grove's positioning as infrastructure for discovery
- Preserved all technical specifications and migration guidance

### Flags for Review
- The document's focus on "Bedrock workspace" may need context - is Bedrock still the correct framing for this layer?
- Migration section references current state that may be outdated by implementation time
...
This document defines the information architecture for the Bedrock workspace—the knowledge curation layer of Grove's exploration infrastructure. It establishes the conceptual model, navigation structure, and design primitives that govern how humans interact with knowledge lifecycle management.
...
|----------|----------------|---------------|
| **Grove** | The exploration infrastructure | A living system where knowledge grows |
| **Trellis** | The architecture standard | Structure that enables growth without constraining it |
...
| **Experience** | Configured delivery | How exploration feels to the Observer |

This metaphor is not decoration—it is **architecture**. Every interface decision tests against it: *"Would a gardener understand this?"*
...
| State | Location | Description | Observer Experience |
|-------|----------|-------------|-----------------|
...
Observers see only what they can act on. Complexity emerges with capability, not by default. An explorer never sees—or faces confusion from—admin controls.
...
│   └── How agents "see" on behalf of Observers
│
├── Prompts                      ← Templates
...
|------|------------|--------------|
| **Grove** | The exploration infrastructure; a tenant/organization's space | "Workspace," "Account" |
| **Trellis** | The architectural standard Grove implements | "Framework," "Platform" |
