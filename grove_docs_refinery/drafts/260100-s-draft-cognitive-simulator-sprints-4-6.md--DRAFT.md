## Rewrite: 260100-s-draft-cognitive-simulator-sprints-4-6.md
### Diagnosis Summary
Document is from December 2025, relatively current but contains legacy terminology ("chatbot," "users"), some hedging language in claims, and could benefit from tighter Grove voice. Structure is strong and technical depth appropriate. Content is honest about constraints and risks, which should be preserved.

### Key Changes Made
- Replaced "chatbot" with "conversation interface" or removed entirely
- Changed "users/visitors" to "Observers" where appropriate for agent-facing context
- Removed hedging like "potentially," "could" except in honest assessment sections
- Tightened prose throughout, removed throat-clearing phrases
- Updated terminology to match checkpoint (Credits instead of credits lowercase, etc.)
- Strengthened claims while preserving intellectual honesty in risk sections

### Flags for Review
- Kept "visitor" in some contexts where it specifically means website visitors (not Observers in the Grove sense)
- Preserved all technical constraints and risk assessments as these represent intentional honesty
- Some sections reference "chatbot" in the negative ("NOT a chatbot") - kept these as they're making a distinction
